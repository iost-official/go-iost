...
function checkGasKeyword(tokens) {
    for (var i = 0; i < tokens.length; i++) {
        if ((tokens[i].type == "Identifier" || tokens[i].type == "Literal") && (tokens[i].value == "_IOSTInstruction_counter.incr")) {
            throw new Error("use of _Gas_Counter keyword is not allowed")
        }
    }
}
...
function addInjection(pos, type, value) {
...
function addInjectionPoint(node, type, value) {
    if (node && node !== null && node.type !== "EmptyExpression") {
...
function ensure_block(node) {
    ...
    if (node.type != "BlockStatement") {
        addInjection(node.range[0], InjectType.leftBrace);
        addInjection(node.range[1], InjectType.rightBrace);
    }
}
...
function ensure_bracket(node) {
    ...
    addInjection(node.range[0], InjectType.leftBracket);
    addInjection(node.range[1], InjectType.rightBracket);
}

function processNode(node, lastInjection) {
    ...
    if (node.type == "IfStatement") {
        ...
        var injectionPoint2 = addInjectionPoint(node.test, InjectType.gasIncrWithComma, 1);
        ...
        if (body.type !== 'BlockStatement') {
            var injectionPoint = addInjectionPoint(node.body, InjectionType.gasIncrWithSemicolon, 0);
            addInjection(node.body.range[0], InjectionType.ret, 0);
...
function traverse(node, path, lastInjection) {
    var newLastInjection;
    var childInjection;
    [newLastInjection, childInjection] = processNode(node, lastInjection);
    ...
    for (var key in node) {
        if (node.hasOwnProperty(key)) {
            ...
            if (typeof child === 'object' && child !== null) {
                ...
                traverse(child, path, keyInjection);
            }
        }
    }
}
