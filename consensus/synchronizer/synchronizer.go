...
type Synchronizer interface {
	Start() error
	Stop()
	CheckSingleBlock(neight int64) bool
	CheckGenBlock() bool
...
func (sy *SyncImpl) CheckSingleBlock(netHeight int64) bool {
...
func (dc *DownloadControllerImpl) OnTimeout(hash string, peerID p2p.PeerID) {
	ilog.Debugf("sync timout, hash=%v, peerID=%v", []byte(hash), peerID.Pretty())
	...
	for {
		...
		case <-dc.chDownload:
			dc.peerMap.Range(func(k, v interface{}) bool {
				...
				ilog.Debugf("peer: %s, state: %s", peerID, pState)
				if !pok {
					return true
				} else {
					ps, ok := pState.(string)
