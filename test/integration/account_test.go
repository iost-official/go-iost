...
func TestAccountInfo(t *testing.T) {
	...
	Convey("test of Auth", t, func() {
		...
		r, err = s.Call("auth.iost", "dropPermission", array2json([]interface{}{"myidid", "perm1"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		So(r.Status.Message, ShouldEqual, "")
		...
		r, err = s.Call("auth.iost", "assignPermission", array2json([]interface{}{"myidid", "active", "@acc1", 1}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		So(r.Status.Message, ShouldContainSubstring, "unexpected item")

		r, err = s.Call("auth.iost", "assignPermission", array2json([]interface{}{"myidid", "active", "IOST1234", 1}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		So(database.Unmarshal(s.Visitor.MGet("auth.iost-auth", "myidid")), ShouldContainSubstring, `{"id":"IOST1234","is_key_pair":true,"weight":1}`)

		r, err = s.Call("auth.iost", "assignPermission", array2json([]interface{}{"myidid", "active", "acc1@active", 1}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		...
		r, err = s.Call("auth.iost", "revokePermission", array2json([]interface{}{"myidid", "active", "acc1@active"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		...
		r, err = s.Call("auth.iost", "revokePermission", array2json([]interface{}{"myidid", "active", "acc2"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		So(r.Status.Message, ShouldContainSubstring, "item not found")

		r, err = s.Call("auth.iost", "revokePermission", array2json([]interface{}{"myidid", "active", "IOST1234"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		...
		r, err = s.Call("auth.iost", "addGroup", array2json([]interface{}{"myidid", "grp0"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		...
		r, err = s.Call("auth.iost", "assignGroup", array2json([]interface{}{"myidid", "grp0", "acc1@active", 1}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		...
		r, err = s.Call("auth.iost", "revokeGroup", array2json([]interface{}{"myidid", "grp0", "acc1@active"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		...
		r, err = s.Call("auth.iost", "assignPermissionToGroup", array2json([]interface{}{"myidid", "active", "grp0"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		So(database.Unmarshal(s.Visitor.MGet("auth.iost-auth", "myidid")), ShouldContainSubstring, `"groups":["grp0"]`)

		r, err = s.Call("auth.iost", "revokePermissionInGroup", array2json([]interface{}{"myidid", "active", "grp0"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
		So(database.Unmarshal(s.Visitor.MGet("auth.iost-auth", "myidid")), ShouldNotContainSubstring, `"groups":["grp0"]`)

		r, err = s.Call("auth.iost", "dropGroup", array2json([]interface{}{"myidid", "grp0"}), acc.ID, acc.KeyPair, signers)
		So(err, ShouldBeNil)
