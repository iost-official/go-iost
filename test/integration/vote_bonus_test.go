...
func TestRebuild(t *testing.T) {
	...
	s.Visitor.SetTokenBalance("iost", acc1.ID, 1e17)
	for idx, acc := range testAccounts {
		if idx == 0 {
			continue
		}
		voter := acc0
		r, err := s.Call("vote_producer.iost", "vote", fmt.Sprintf(`["%v", "%v", "%v"]`, voter.ID, acc.ID, 2100000), voter.ID, voter.KeyPair)
		assert.Nil(t, err)
		assert.Empty(t, r.Status.Message)
	...
	r, err := s.Call("vote_producer.iost", "vote", fmt.Sprintf(`["%v", "%v", "%v"]`, acc0.ID, acc0.ID, 2000000), acc0.ID, acc0.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	...
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	assert.Equal(t, int64(185185189), s.Visitor.TokenBalance("iost", acc6.ID)) // 166666670 = (3333333410*(1/9))/2

	r, err = s.Call("vote_producer.iost", "voterWithdraw", fmt.Sprintf(`["%s"]`, acc0.ID), acc0.ID, acc0.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	assert.Equal(t, int64(92592594), s.Visitor.TokenBalance("iost", acc0.ID))

	r, err = s.Call("vote_producer.iost", "rebuild", `[]`, acc0.ID, acc0.KeyPair)
	assert.Nil(t, err)

	r, err = s.Call("vote_producer.iost", "candidateWithdraw", fmt.Sprintf(`["%s"]`, acc5.ID), acc5.ID, acc5.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	assert.Equal(t, int64(0), s.Visitor.TokenBalance("iost", acc5.ID))

	r, err = s.Call("vote_producer.iost", "voterWithdraw", fmt.Sprintf(`["%s"]`, acc1.ID), acc1.ID, acc1.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	assert.Equal(t, int64(0), s.Visitor.TokenBalance("iost", acc1.ID))

	s.Head.Time += 24*3600*1e9 + 1
	r, err = s.Call("issue.iost", "issueIOST", `[]`, acc0.ID, acc0.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	s.Visitor.SetTokenBalance("iost", acc1.ID, 0)

	r, err = s.Call("vote_producer.iost", "candidateWithdraw", fmt.Sprintf(`["%s"]`, acc5.ID), acc5.ID, acc5.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	assert.Equal(t, int64(3153729205195), s.Visitor.TokenBalance("iost", acc5.ID)) // 56767125693515/18

	assert.Equal(t, int64(0), s.Visitor.TokenBalance("iost", acc1.ID))
	r, err = s.Call("vote_producer.iost", "voterWithdraw", fmt.Sprintf(`["%s"]`, acc1.ID), acc1.ID, acc1.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	assert.Equal(t, int64(1576864602597), s.Visitor.TokenBalance("iost", acc1.ID))

	s.Visitor.SetTokenBalance("iost", acc1.ID, 1e17)
	r, err = s.Call("vote_producer.iost", "vote", fmt.Sprintf(`["%v", "%v", "%v"]`, acc1.ID, acc0.ID, 100000), acc1.ID, acc1.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	...
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	s.Visitor.SetTokenBalance("iost", acc0.ID, 0)
	s.Visitor.SetTokenBalance("iost", acc1.ID, 0)

	r, err = s.Call("vote_producer.iost", "candidateWithdraw", fmt.Sprintf(`["%s"]`, acc0.ID), acc0.ID, acc0.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	assert.Equal(t, int64(1493992875634), s.Visitor.TokenBalance("iost", acc0.ID)) // 56767125693515/38

	assert.Equal(t, int64(0), s.Visitor.TokenBalance("iost", acc1.ID))
	r, err = s.Call("vote_producer.iost", "voterWithdraw", fmt.Sprintf(`["%s"]`, acc1.ID), acc1.ID, acc1.KeyPair)
	assert.Nil(t, err)
	assert.Empty(t, r.Status.Message)
	assert.Equal(t, int64(71142517887), s.Visitor.TokenBalance("iost", acc1.ID))

	kp, err := account.NewKeyPair(common.Base58Decode("546aCDG9igGgZqVZeybajaorP5ZeF9ghLu2oLncXk3d6"), crypto.Secp256k1)
	assert.Nil(t, err)
	for i := 0; i < 20; i++ {
