...
func prepareToken(t *testing.T, s *Simulator, pubAcc *TestAccount) {
	r, err := s.Call("token.iost", "create", fmt.Sprintf(`["%v", "%v", %v, {}]`, "iost", testAccounts[0].ID, "21000000000"), pubAcc.ID, pubAcc.KeyPair)
...
func Test_NewVote(t *testing.T) {
	...
	Convey("test NewVote", t, func() {
		...
		Convey("test NewVote", func() {
			r, err := prepareVote(t, s, acc)

			So(err, ShouldBeNil)
			So(r.Status.Code, ShouldEqual, tx.Success)
			So(s.Visitor.TokenBalance("iost", acc.ID), ShouldEqual, int64(1999999000*1e8))
			...
			r, err = s.Call("Contractvoteresult", "GetResult", `["1"]`, acc.ID, acc.KeyPair)
...
func Test_AddOption(t *testing.T) {
	...
	Convey("test vote", t, func() {
		...
		Convey("test AddOption", func() {
			r, err := s.Call("vote.iost", "AddOption", `["1", "option5", true]`, acc.ID, acc.KeyPair)
			...
			r, err = s.Call("vote.iost", "GetOption", `["1", "option5"]`, acc.ID, acc.KeyPair)
			So(err, ShouldBeNil)
			So(r.Status.Code, ShouldEqual, tx.Success)
...
func Test_RemoveOption(t *testing.T) {
	...
	Convey("test vote", t, func() {
		...
		Convey("test RemoveOption", func() {
			r, err := s.Call("vote.iost", "RemoveOption", `["1", "option2", true]`, acc.ID, acc.KeyPair)
...
func Test_Vote(t *testing.T) {
	...
	Convey("test vote", t, func() {
		...
		prepareVote(t, s, acc1)

		Convey("test Vote", func() {
			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option3", "5"]`, acc2.ID), acc2.ID, acc2.KeyPair)

			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-v_1", "option3")), ShouldEqual, `["5",false,-1]`)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option3":["5",0,"0"]}`)
			...
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option3":["10",0,"0"]}`)
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{"option3"})

			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option1", "20"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-v_1", "option1")), ShouldEqual, `["20",false,-1]`)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option3":["10",0,"0"],"option1":["20",0,"0"]}`)
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{"option3", "option1"})

			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option3", "100"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-v_1", "option3")), ShouldEqual, `["110",false,-1]`)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc1.ID)), ShouldEqual, `{"option3":["100",0,"0"]}`)
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{"option3", "option1"})

			s.Call("Contractvoteresult", "GetResult", `["1"]`, acc1.ID, acc1.KeyPair)
			...
			r, err := s.Call("vote.iost", "GetOption", `["1", "option3"]`, acc1.ID, acc1.KeyPair)
			So(err, ShouldBeNil)
			So(r.Status.Code, ShouldEqual, tx.Success)
			So(r.Returns[0], ShouldEqual, `["{\"votes\":\"110\",\"deleted\":false,\"clearTime\":-1}"]`)

			r, err = s.Call("vote.iost", "GetVote", fmt.Sprintf(`["1", "%v"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(err, ShouldBeNil)
			So(r.Status.Code, ShouldEqual, tx.Success)
		...
		Convey("test Unvote", func() {
			// vote
			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option1", "100"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{"option1"})

			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option2", "100"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			...
			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option3", "300"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{"option1", "option2", "option3"})

			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option4", "400"]`, acc3.ID), acc3.ID, acc3.KeyPair)
			...
			// unvote
			s.Call("vote.iost", "Unvote", fmt.Sprintf(`["1", "%v", "option3", "100"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-v_1", "option3")), ShouldEqual, `["200",false,-1]`)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc1.ID)), ShouldEqual, `{"option2":["100",0,"0"],"option3":["200",0,"0"]}`)
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{"option1", "option2", "option3", "option4"})
			...
			// unvote again
			s.Call("vote.iost", "Unvote", fmt.Sprintf(`["1", "%v", "option2", "95"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-v_1", "option2")), ShouldEqual, `["105",false,-1]`)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc1.ID)), ShouldEqual, `{"option2":["5",0,"0"],"option3":["200",0,"0"]}`)
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{"option1", "option2", "option3", "option4"})

			s.Call("vote.iost", "Unvote", fmt.Sprintf(`["1", "%v", "option2", "100"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-v_1", "option2")), ShouldEqual, `["5",false,-1]`)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option1":["100",0,"0"]}`)
...
func Test_DelVote(t *testing.T) {
	...
	Convey("test DelVote", t, func() {
		...
		Convey("test delete vote", func() {
			...
			// del vote
			s.Call("vote.iost", "DelVote", `["1"]`, acc1.ID, acc1.KeyPair)
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{})
			...
			So(s.Visitor.MKeys("vote.iost-p_1"), ShouldResemble, []string{})
			So(s.Visitor.MKeys("vote.iost-v_1"), ShouldResemble, []string{})
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc1.ID)), ShouldEqual, `{"option2":["5",0,"0"],"option3":["300",0,"0"]}`)

			// unvote all
			s.Call("vote.iost", "Unvote", fmt.Sprintf(`["1", "%v", "option2", "5"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			s.Call("vote.iost", "Unvote", fmt.Sprintf(`["1", "%v", "option3", "300"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			So(s.Visitor.MHas("vote.iost-u_1", acc1.ID), ShouldEqual, false)
		})
	})
}
...
func Test_MixVoteOption(t *testing.T) {
	...
	Convey("test mixed", t, func() {
		...
		Convey("test AddOption not clear", func() {
			...
			// add option
			s.Call("vote.iost", "AddOption", `["1", "option5", false]`, acc1.ID, acc1.KeyPair)
			So(s.Visitor.MKeys("vote.iost-v_1"), ShouldResemble, []string{"option1", "option2", "option3", "option4", "option5"})
			...
			s.Head.Number++
			// remove option
			s.Call("vote.iost", "RemoveOption", `["1", "option1", false]`, acc1.ID, acc1.KeyPair)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-v_1", "option1")), ShouldEqual, `["100",true,-1]`)

			// add option
			s.Call("vote.iost", "AddOption", `["1", "option1", false]`, acc1.ID, acc1.KeyPair)
		...
		Convey("test AddOption and clear", func() {
			// vote
			rs, err := s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option1", "100"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			rs, err = s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option2", "100"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			rs, err = s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option2", "100"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			rs, err = s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option3", "200"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			rs, err = s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option4", "400"]`, acc3.ID), acc3.ID, acc3.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")

			s.Head.Number++
			// remove option
			rs, err = s.Call("vote.iost", "RemoveOption", `["1", "option1", false]`, acc1.ID, acc1.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			// add option
			rs, err = s.Call("vote.iost", "AddOption", `["1", "option1", true]`,acc1.ID, acc1.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			...
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option1":["200",1,"100"],"option2":["100",0,"0"]}`)
			...
			So(rs.Status.Message, ShouldEqual, "")
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-p_1", "option1")), ShouldEqual, `"100"`)
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option1":["300",2,"200"],"option2":["100",0,"0"]}`)

			// vote again
			rs, err = s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option1", "100"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option1":["400",2,"200"],"option2":["100",0,"0"]}`)

			// get result
			rs, err = s.Call("Contractvoteresult", "GetResult", `["1"]`,acc1.ID, acc1.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			So(database.MustUnmarshal(s.Visitor.MGet("Contractvoteresult-vote_result", "1")), ShouldEqual, `[{"option":"option4","votes":"400"},{"option":"option2","votes":"200"}]`)

			// unvote
			rs, err = s.Call("vote.iost", "Unvote", fmt.Sprintf(`["1", "%v", "option1", "50"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option1":["350",2,"150"],"option2":["100",0,"0"]}`)

			// unvote again
			rs, err = s.Call("vote.iost", "Unvote", fmt.Sprintf(`["1", "%v", "option1", "200"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			So(err, ShouldBeNil)
			So(rs.Status.Message, ShouldEqual, "")
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-u_1", acc2.ID)), ShouldEqual, `{"option1":["150",2,"0"],"option2":["100",0,"0"]}`)
		...
		Convey("test RemoveOption not force", func() {
			// vote
			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option1", "100"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option2", "200"]`, acc2.ID), acc2.ID, acc2.KeyPair)
			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option2", "100"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option3", "300"]`, acc1.ID), acc1.ID, acc1.KeyPair)
			s.Call("vote.iost", "Vote", fmt.Sprintf(`["1", "%v", "option4", "400"]`, acc3.ID), acc3.ID, acc3.KeyPair)
			...
			// remove option
			s.Call("vote.iost", "RemoveOption", `["1", "option5", false]`,acc1.ID, acc1.KeyPair)
			// s.Call("vote.iost", "RemoveOption", `["1", "option4", false]`,acc1.ID, acc1.KeyPair) // should fail
			// s.Call("vote.iost", "RemoveOption", `["1", "option3", false]`,acc1.ID, acc1.KeyPair) // should fail
			// s.Call("vote.iost", "RemoveOption", `["1", "option2", false]`,acc1.ID, acc1.KeyPair) // should fail
			s.Call("vote.iost", "RemoveOption", `["1", "option1", false]`,acc1.ID, acc1.KeyPair)
			So(s.Visitor.MKeys("vote.iost-v_1"), ShouldResemble, []string{"option1", "option2", "option3", "option4", "option5"})
		...
		Convey("test RemoveOption with force", func() {
			...
			// remove option
			s.Call("vote.iost", "RemoveOption", `["1", "option5", true]`,acc1.ID, acc1.KeyPair)
			s.Call("vote.iost", "RemoveOption", `["1", "option4", true]`,acc1.ID, acc1.KeyPair)
			s.Call("vote.iost", "RemoveOption", `["1", "option3", true]`,acc1.ID, acc1.KeyPair)
			s.Call("vote.iost", "RemoveOption", `["1", "option2", true]`,acc1.ID, acc1.KeyPair)
			s.Call("vote.iost", "RemoveOption", `["1", "option1", true]`,acc1.ID, acc1.KeyPair)
			So(s.Visitor.MKeys("vote.iost-v_1"), ShouldResemble, []string{"option1", "option2", "option3", "option4", "option5"})
			So(database.MustUnmarshal(s.Visitor.MGet("vote.iost-v_1", "option5")), ShouldEqual, `["0",true,-1]`)
